...
check "GBrain CLI" "command -v gbrain >/dev/null 2>&1 || test -f ~/.bun/bin/gbrain"

# Database
check "organizer.db exists" "test -f ~/.autognosia/personal-organizer/data/organizer.db"

# Wiki structure
check "Active Wiki directory" "test -d ~/.autognosia/active-wiki"
check "Active Wiki projects" "test -d ~/.autognosia/active-wiki/projects"
