...
When performing web searches during graphify operations, use this priority order:

1. **CamoFox** (self-hosted Docker browser extraction) — primary, unlimited
2. **Firecrawl** (self-hosted Docker) - http://127.0.0.1:3002
3. **Playwright** (self-hosted Docker browser extraction)
4. **SearXNG** (self-hosted, free, unlimited) - http://127.0.0.1:8080
5. **Tavily** (LAST RESORT - limited API calls/month)

Tavily must only be used when all self-hosted options fail. See `search-priority.md` in the Autognosia repo for details.
...
When graphify operations take >60s (typical for corpora >20k nodes), the terminal gateway will kill the process. **Workaround:** use `execute_code()` with `subprocess.run()` and the correct Python interpreter path instead of `terminal()`. The graphifyy interpreter lives at `/home/home_user/.local/share/uv/tools/graphifyy/bin/python3` — use that as the subprocess command. This bypasses the terminal gateway's timeout on long-running scripts.

Example:
```python
import subprocess
subprocess.run(
    ['/home/home_user/.local/share/uv/tools/graphifyy/bin/python3', '-c', '...script...'],
    capture_output=True, text=True, timeout=120, cwd='/path/to/corpus'
)
```

## Subagent Dispatch Pitfall

When dispatching subagents for semantic extraction, do NOT embed large file lists directly in the task goal string. If the goal contains hundreds of file paths, the dispatch can timeout (420s). Instead:
- Write file lists to external text files (one path per line)
- Reference the file in the goal: "Read files from /path/to/filelist.txt"
- Have the subagent read and process the files itself

## ⚠️ Schema Consistency — Subagent Extraction Pitfall (CRITICAL)

**Problem:** Subagents writing their own extraction scripts (`extract_chunk_XX.py`) produce wildly inconsistent schemas — multiple different node/edge field names, missing required fields, custom field names, no confidence scores, no source_file. In a real run, **59% of chunks were non-compliant**, with **13 different schemas** across 27 chunks. This breaks the merge pipeline because the graphifyy merge step expects the standardized schema from `extraction-spec.md`.

**Root cause:** Subagents that don't receive the full extraction-spec prompt write their own ad-hoc extraction scripts instead of using the graphifyy Python library's `extract()` function.

**How to avoid:**
1. **Dispatch ALL subagents in a single message** — one call per chunk, all in one response. This is the only way they run in parallel.
2. **Give each subagent the exact extraction-spec prompt** (load `references/extraction-spec.md` and pass it verbatim with FILE_LIST, CHUNK_NUM, TOTAL_CHUNKS, and CHUNK_PATH substituted). Do NOT let subagents "figure out the schema themselves."
3. **Require the standard schema** — the subagent prompt must include the exact JSON output template from extraction-spec.md. No exceptions.
4. **After dispatch, verify chunks** — before merging, check each `.graphify_chunk_XX.json` for the required top-level keys (`nodes`, `edges`, `hyperedges`, `input_tokens`, `output_tokens`), node fields (`id`, `label`, `file_type`, `source_file`), and edge fields (`source`, `target`, `relation`, `confidence`, `confidence_score`). Skip non-compliant chunks and re-dispatch if needed.
5. **Prefer `graphifyy.extract()` when possible** — the Python library produces correct schema automatically. Use it for code-only corpora (AST extraction) and when Gemini is available (semantic extraction).
6. **Post-merge schema validation** — after merging all chunks into `.graphify_semantic.json`, run a quick validation pass. If >50% of chunks are non-compliant, the merge will produce a broken graph. Re-dispatch failed chunks with a corrected prompt.

**What non-compliance looks like in practice:**
- `type` instead of `file_type`
- `name` instead of `label`
- `source_paths` (array) instead of `source_file` (string)
- `properties` dict instead of structured fields
- Missing `confidence` / `confidence_score` entirely
- Missing `hyperedges`, `input_tokens`, `output_tokens`
- Invalid `confidence_score` of 0.5 (forbidden by spec)
- Custom relation names like `related_to`, `mentored`, `co-organized_with`
- Format drift within the same pipeline run

See `references/subagent-extraction-schemas.md` for the full schema audit and compliance breakdown.
