...
When graphify operations take >60s (typical for corpora >20k nodes), the terminal gateway will kill the process. **Workaround:** use `execute_code()` with `subprocess.run()` and the correct Python interpreter path instead of `terminal()`. The graphifyy interpreter lives at `/home/<USER>/.local/share/uv/tools/graphifyy/bin/python3` — use that as the subprocess command. This bypasses the terminal gateway's timeout on long-running scripts.

Example:
```python
import subprocess
subprocess.run(
    ['/home/<USER>/.local/share/uv/tools/graphifyy/bin/python3', '-c', '...script...'],
