...
```bash
# 1. Copy the project into a scratch workdir
WORK=/tmp/oc-<project>-$(date -u +%Y%m%dT%H%M%SZ)
mkdir -p "$WORK" && cp -r /path/to/original/. "$WORK/"

# 2. Point OpenCode at the COPY only
cd "$WORK" && opencode run '<task>'

# 3. Review IN THE COPY — read it, run it, test it
# 4. Only after review passes, copy approved files back one at a time
```

Never `cd` into the real project and run OpenCode there. Never pass the live path.
If OpenCode destroys the scratch dir, that costs nothing — that is the entire point.

### Verify the originals afterwards
...
```
$ opencode run 'Respond with exactly: OPENCODE_LMSTUDIO_OK'
> build · qwen3.8-27b
OPENCODE_LMSTUDIO_OK
```

It writes files, runs them, and reports back. Confirmed end-to-end with a real
code task, then independently re-verified by me.
