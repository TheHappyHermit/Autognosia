...
Old Docker containers that are stopped/removed can leave orphaned `llama-server` processes running as root, holding VRAM. These don't show in `docker ps` but appear in `nvidia-smi`. Fix: `sudo kill -9 $(pgrep llama-server)` or reboot. Sudoers NOPASSWD configured at `/etc/sudoers.d/<USER>-nogpass`.