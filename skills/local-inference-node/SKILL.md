...
- **`sudo -S` is blocked by the terminal tool** — cannot pipe passwords. Sudoers NOPASSWD file at `/etc/sudoers.d/home_user-nogpass` covers kill, pkill, nvidia-smi, docker, systemctl, shutdown, reboot. Use `sudo kill` directly over SSH, not `echo pw | sudo -S`.
