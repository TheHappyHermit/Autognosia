...
|-------|-------------|-------------|------------|-----------------|
| **Main Graph** | `~/.hermes-cortex/active-wiki/` | Hot/current | Main Hermes, Planner | Frequent (days/weeks) |
| **Oracle Graph** | `~/.hermes-cortex/oracle/brain/` | Cold/historical | Oracle Profile | Infrequent (weeks/months) |
...
1. Ordinary wiki search (`ripgrep ~/.hermes-cortex/active-wiki/`)
2. Direct Markdown/source inspection
3. Oracle (if appropriate)
4. Research Hermes

### Oracle Graph Fallback Order
1. Ordinary wiki search (`ripgrep ~/.hermes-cortex/oracle/brain/`)
2. Direct Markdown/source inspection
3. GBrain semantic/hybrid retrieval (via Oracle profile)
4. Raw evidence search (`~/.hermes-cortex/oracle/raw/`)
5. Research Hermes
...
rm -rf ~/.hermes-cortex/graphify-main-out
graphify extract ~/.hermes-cortex/active-wiki \
  --backend openai --model "your-model-id" \
  --token-budget 50000 --max-concurrency 4 --no-cluster --api-timeout 1200 \
  --out ~/.hermes-cortex/graphify-main-out

# Incremental (AST only)
graphify update ~/.hermes-cortex/active-wiki --graph ~/.hermes-cortex/graphify-main-out
```

### Oracle Graph (Oracle Wiki)
```bash
# Full semantic run (cache clear MANDATORY)
rm -rf ~/.hermes-cortex/graphify-oracle-out
graphify extract ~/.hermes-cortex/oracle/brain \
  --backend openai --model "your-model-id" \
  --token-budget 50000 --max-concurrency 4 --no-cluster --api-timeout 1200 \
  --out ~/.hermes-cortex/graphify-oracle-out

# Incremental (AST only)
graphify update ~/.hermes-cortex/oracle/brain --graph ~/.hermes-cortex/graphify-oracle-out
```
...
```bash
# Extract
graphify extract ~/.hermes-cortex/active-wiki --backend openai --model "your-model-id"

# Query
graphify query "How does X connect to Y?" --graph ~/.hermes-cortex/graphify-main-out

# Update (incremental)
graphify update ~/.hermes-cortex/active-wiki --graph ~/.hermes-cortex/graphify-main-out

# Explain
graphify explain "concept-name" --graph ~/.hermes-cortex/graphify-main-out

# Path
graphify path "node-a" "node-b" --graph ~/.hermes-cortex/graphify-main-out
```
...
ls -la --time-style=full-iso ~/.hermes-cortex/graphify-main-out/GRAPH_REPORT.md

# 2. Semantic cache files in thousands
find ~/.hermes-cortex/graphify-main-out/cache/semantic -type f | wc -l

# 3. Log shows semantic extraction on ~total files
grep "semantic extraction on" ~/.hermes-cortex/graphify-main-out/*.log
```
...
```bash
graphify query "Trace compliance flow from A to B" --graph ~/.hermes-cortex/graphify-oracle-out
graphify explain "historical-concept" --graph ~/.hermes-cortex/graphify-oracle-out
graphify path "node-x" "node-y" --graph ~/.hermes-cortex/graphify-oracle-out
```
