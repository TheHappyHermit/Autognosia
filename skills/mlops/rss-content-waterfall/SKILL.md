...
**When to use**: The FreshRSS hostname is NXDOMAIN, not in `/etc/hosts`, or only resolves on a different network segment. Use the raw server IP (e.g., `<V100_HOST>`) with the correct `Host` header (e.g., `freshrss.<oracle-server>`).
