...
Verified 2026-08-16. Repo: `~/hermes-cortex` (github.com/<email>/hermes-cortex). README is the design doc (~1900 lines): retrieval-cost hierarchy Honcho → Active Wiki → Oracle → GBrain; knowledge flows hot→cold, never deleted ("knowledge changes temperature").

## Canonical paths (`config/paths.yaml`)
- `cortex_home`: `~/.hermes-cortex`
- Active Wiki: `~/.hermes-cortex/active-wiki` (subdirs: personal/projects/reference/system)
- Oracle: `oracle_path: ~/.hermes-cortex/oracle`
- organizer.db: `~/.hermes-cortex/personal-state/data/organizer.db`
- Backups: `~/.hermes-cortex/backups`

## CRITICAL MISMATCH — what scripts actually read
`paths.yaml` says oracle root, but operational components read DEEPER. Dropping files at `~/.hermes-cortex/oracle/` makes them **invisible to the entire pipeline**:
...
1. Oracle Vault moved into `~/.hermes-cortex/oracle/brain/` — 379 md; `oracle/` now contains only `brain/`.
2. Copied to `~/personal-agent/oracle/brain` — 379 md (GBrain Sync + oracle_search.py fallback now have content).
3. LLM wiki: 140 md copied into `~/.hermes-cortex/active-wiki/`; old location preserved as `~/wiki.bak-20260816`; `WIKI_PATH` appended to `.env`.
4. Seven desktop skills pushed path-adapted (25 files, zero Windows paths remaining): oracle-query + hermes-troubleshooting (with nested cron-job-management) at top level; llm-wiki-commands, oracle-entity-creation, oracle-wiki-research-pipeline, wiki-maintenance-hermes under `research/`. Mapping used: Vault→`~/.hermes-cortex/oracle/brain`, LLM_WIKI→`active-wiki`, Oracle\Incoming→`~/.hermes-cortex/incoming` (dir created), Backups/Daily→`~/backups/`, AppData logs/cron/state.db→`~/.hermes/...`, `.hermes.md` enforcement ref→`SYSTEM-RULES.md`. Visible to the remote instance only after its gateway reloads.

## Data copied from Windows desktop (2026-08-16)
- Oracle Vault: 379 md → `~/.hermes-cortex/oracle/brain/` (stray literal `~` dir excluded — accidental clone of the Oracle-Vault GitHub repo, 731 md; designed home if ever preserved: `oracle/raw/`)
- LLM_WIKI: 140 md → `~/.hermes-cortex/active-wiki/` (originally dropped at `~/wiki`, moved same day)
