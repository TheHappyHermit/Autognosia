...
- Active Wiki: /home/<USER>/.autognosia/active-wiki (WIKI_PATH in ~/.bashrc)
- Oracle brain: /home/<USER>/.autognosia/oracle/brain
...
- Active Wiki: `~/.hermes-autognosia/active-wiki` (subdirs: personal/projects/reference/system)
- Oracle: `oracle_path: ~/.hermes-autognosia/oracle`
- organizer.db: `~/.hermes-autognosia/personal-state/data/organizer.db`
- Backups: `~/.hermes-autognosia/backups`

## CRITICAL MISMATCH — what scripts actually read
`paths.yaml` says oracle root, but operational components read DEEPER. Dropping files at `~/.hermes-autognosia/oracle/` makes them **invisible to the entire pipeline**:
...
The agent VM's own Hermes instance auto-created **23 cron jobs** at 21:29 on 2026-08-16 from `hermes-autognosia/cron-jobs/definitions.md` (Config/Database Backup, Integrity Check, Oracle Index Rebuild, Wiki Lint Daily+Weekly, Memory Consolidation, View Generation, Briefing, Intention/Prompt-Me checks, Weekly/Monthly Reviews, Graphify Refresh, Experience Capture, Session/Persona Audit, Autognosia Health Check, Research Exchange Sync). Total: 30 jobs. **Do not assume the job list is static — re-list before any audit.**
...
- Oracle Vault: 379 md → `~/.hermes-autognosia/oracle/brain/` (stray literal `~` dir excluded — accidental clone of the Oracle-Vault GitHub repo, 731 md; designed home if ever preserved: `oracle/raw/`)
- LLM_WIKI: 140 md → `~/.hermes-autognosia/active-wiki/` (originally dropped at `~/wiki`, moved same day)
