...
Audit and sync skills, cron jobs, and knowledge bases across the user's Hermes installs (Windows desktop ↔ agent VM 10.1.1.37) over SSH.
...
- **Agent VM 10.1.1.37** (user <username>) — runs hermes-cortex; its own Hermes instance operates autonomously 24/7.
...
3. **Path mapping BEFORE any transfer.** The destination is whatever the *operational components* read, not what config files claim:
   - Read `hermes-cortex/config/paths.yaml` for declared paths — then grep the actual scripts (`~/hermes-cortex/scripts/*.py`) and installed skills for hardcoded paths. On 10.1.1.37 these disagree (see references/agent-server-cortex-paths.md).
   - Check env vars that redirect defaults: e.g. bundled llm-wiki skill reads `WIKI_PATH` (unset → `~/wiki`; on 10.1.1.37 it is now set to the cortex active-wiki — see reference).
...
- **tar-over-SSH pipe** — rsync is not installed on the agent VM; tar exists both sides:
  ```bash
  tar -C /c/Hermes/Oracle/Vault --exclude='./~' -cf - . | ssh -i ~/.ssh/id_ed25519_agent_server <username>@10.1.1.37 "mkdir -p <dest> && tar -xf - -C <dest>"
  ```
...
- **Quoting hell with nested SSH + python.** `ssh host "python3 -c \"...\""` mangles quotes through git-bash. Instead write the script locally and pipe via stdin:
  ```bash
  cat /tmp/check.py | ssh -i ~/.ssh/id_ed25519_agent_server <username>@10.1.1.37 "python3 -"
  ```
...
- `references/agent-server-cortex-paths.md` — verified path map for 10.1.1.37 (paths.yaml vs actual script paths, GBrain hardcodes, cron self-provisioning behavior, what was copied when).
