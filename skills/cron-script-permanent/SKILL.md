...
description: Manage Hermes cron jobs — convert transient temp-file scripts to permanent ones, and maintain them in ${HOME}/.hermes/scripts/ and the repo.
---

# Cron Script Management

Use when creating or maintaining Hermes cron jobs that execute scripts. This covers both local cron jobs (`${HOME}/.hermes/scripts/`) and repo scripts (`autognosia-repo/scripts/`).
...
1. **Write the script to a permanent location:**
   - Local scripts: `${HOME}/.hermes/scripts/<name>.py`
...
- **Cron jobs may error before the script is installed.** If a cron job references a script that doesn't exist in `${HOME}/.hermes/scripts/`, it fails silently. Always check `last_status` after updating.
...
1. Identify the cron job creating transient scripts (check `${HOME}/.hermes/cron/output/` for recent runs)
