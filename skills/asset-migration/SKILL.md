...
- **Legacy repo contamination:** A legacy repo (e.g., `hermes-cortex/`) may contain old branded images. Copying them overwrites newer versions in the target repo (e.g., `autognosia-repo/`). The result is all images suddenly show the old brand name.
...
- See `references/legacy-repo-trust.md` for the hermes-cortex → autognosia case study.
