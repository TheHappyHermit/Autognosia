...
|-------|-------------|-------------|------------|-----------------|
| **Main Graph** | `/home/<USER>/.autognosia/active-wiki/` | Hot/current | Main Hermes, Planner | Frequent (days/weeks) |
| **Oracle Graph** | `/home/<USER>/.autognosia/oracle/brain/` | Cold/historical | Oracle Profile | Infrequent (weeks/months) |
...
1. Ordinary wiki search (`ripgrep /home/<USER>/.autognosia/active-wiki/`)
2. Direct Markdown/source inspection
3. Oracle (if appropriate)
4. Research Hermes

### Oracle Graph Fallback Order
1. Ordinary wiki search (`ripgrep /home/<USER>/.autognosia/oracle/brain/`)
2. Direct Markdown/source inspection
3. **Ripgrep fallback** (if semantic search misses)
4. Raw evidence search (`/home/<USER>/.autognosia/oracle/raw/`)
5. Research Hermes
...
# Output lands in: /home/<USER>/.autognosia/active-wiki/graphify-out/
```
...
# Output lands in: /home/<USER>/.autognosia/oracle/brain/graphify-out/
```
...
**no OpenRouter fallback** and **no localhost:1234 (3090)** path — both are
...
```bash
# Active wiki graph
OUT=/home/<USER>/.autognosia/active-wiki/graphify-out
ls -la --time-style=full-iso "$OUT/GRAPH_REPORT.md"
find "$OUT/cache/semantic" -type f | wc -l
grep "semantic extraction on" /home/<USER>/.autognosia/logs/graphify-active-wiki.log

# Oracle brain graph
OUT=/home/<USER>/.autognosia/oracle/brain/graphify-out
ls -la --time-style=full-iso "$OUT/GRAPH_REPORT.md"
find "$OUT/cache/semantic" -type f | wc -l
grep "semantic extraction on" /home/<USER>/.autognosia/logs/graphify-oracle-brain.log
```
...
```bash
graphify query "How does X connect to Y?" --graph /home/<USER>/.autognosia/graphify-main-out
graphify explain "concept-name" --graph /home/<USER>/.autognosia/graphify-main-out
graphify path "node-a" "node-b" --graph /home/<USER>/.autognosia/graphify-main-out
```

### Oracle Graph (Oracle Profile)
```bash
graphify query "Trace compliance flow from A to B" --graph /home/<USER>/.autognosia/graphify-oracle-out
graphify explain "historical-concept" --graph /home/<USER>/.autognosia/graphify-oracle-out
graphify path "node-x" "node-y" --graph /home/<USER>/.autognosia/graphify-oracle-out
```
