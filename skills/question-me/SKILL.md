...
| **Cold** | LLM Wiki (`$HOME/.autognosia/active-wiki\`) | Archived preferences, settled decisions, historical context | File reads + wiki search |
...
4. **Wiki ingestion (full pipeline):**
   ...
   a. **Categorize** — place in `$HOME/.autognosia/active-wiki\system\memory-archive\` with the right file:
   ...
   **Without a source reference, the entry is just a summary with no road back to evidence.**
   ...
   e. **Update wiki index** — add entry to `$HOME/.autognosia/active-wiki\index.md` if the archive section doesn't exist yet
   f. **Update log** — append to `$HOME/.autognosia/active-wiki\log.md`: `## YYYY-MM-DD: memory consolidation | archived X entries from hot memory`
