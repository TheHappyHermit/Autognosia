...
Support files:
- `references/cron-429-pinned-model-no-fallback.md` — Exact error transcript and
  recurrence-grep recipe from a real 429 incident.
- `references/graphify-path-mismatch.md` — Case study of the graphify refresh
  script path mismatch (Pattern 8): "graph not initialized" despite existing
  graph.json, the `graphify-out/` subdirectory convention, and the in-place
  `graphify update` fix.
...
1. Extract the logic from the temp scripts into a permanent script:
   `$HOME/.autognosia/scripts/<name>.py`
2. Make it executable: `chmod +x $HOME/.autognosia/scripts/<name>.py`
3. Update the cron job prompt to reference the permanent script:
   ```
   Run: python3 $HOME/.autognosia/scripts/<name>.py
...
`$HOME/.autognosia/scripts/verify_graphify_integrity.py` and updating
...
**Symptom:** `last_error: "Script not found: $HOME/.hermes/scripts/autognosia_*.py"`
...
## Pattern 8 — graphify "graph not initialized" is a path mismatch, not a missing graph

Graphify's `extract` and `update` commands write `graph.json` to
`<source>/graphify-out/graph.json` (a subdirectory under the source or `--out`
dir). Scripts that check for `graph.json` at `<output_dir>/graph.json` (without
the `graphify-out/` segment) will always report "graph not found" even though
the graph exists.

**Symptom:** Script error: `Main Graph not initialized. Run init_graphify.py
first.` — but `find ~/.autognosia -name "graph.json" -path "*graphify-out*"`
shows the file exists at `graphify-main-out/graphify-out/graph.json` (note the
extra subdirectory).

**Key insight:** Do NOT re-run `init_graphify.py` — the graph already exists.
The script is checking the wrong path. Re-running init triggers a full
re-extraction (expensive — hours through llama.cpp).

**Diagnosis:**
1. `find ~/.autognosia -name "graph.json" -path "*graphify-out*" -exec ls -la {} \;`
   — find where graphify actually writes.
2. Read the refresh script: grep for `graph.json` and note the exact path it
   constructs.
3. Compare: if the script checks `<dir>/graph.json` but graphify writes to
   `<dir>/graphify-out/graph.json`, that's the bug.

**Fix (preferred — run in-place):** Run graphify from within the source
directory with `cwd=source`. graphify writes to `<source>/graphify-out/`
relative to CWD, and the script checks `<source>/graphify-out/graph.json`.
No separate output dir needed — avoids the mismatch entirely.

**Fix (if using separate output dir):** Update the path check to include the
`graphify-out/` segment:
```python
# Wrong — missing the graphify-out/ subdirectory:
graph_file = os.path.join(MAIN_GRAPH_OUT, "graph.json")
# Correct:
graph_file = os.path.join(MAIN_GRAPH_OUT, "graphify-out", "graph.json")
```

**Also note:** `graphify update` does NOT accept `--graph` as an argument
in the current version (0.46.x). It runs in-place from the source directory.
`graphify extract` writes to `<path>/graphify-out/` or `<--out DIR>` and puts
graph.json inside `graphify-out/` under that dir.

See `references/graphify-path-mismatch.md` for the exact reproduction.
...
- For Pattern 8: `ls ~/.autognosia/active-wiki/graphify-out/graph.json` (or
  oracle/brain equivalent) confirms the in-place graph.json exists; running
  the refresh script exits 0 instead of erroring on path mismatch.
