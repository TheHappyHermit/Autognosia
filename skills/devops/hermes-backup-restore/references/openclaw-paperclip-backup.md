...
if [ ! -d .git ]; then
    git init
    git config --local credential.helper '!f() { echo "username=<username>"; echo "password=$(gh auth token)"; }; f'
    git remote add origin https://github.com/<username>/openclaw-customizations.git
fi
...
if [ ! -d .git ]; then
    git init
    git config --local credential.helper '!f() { echo "username=<username>"; echo "password=$(gh auth token)"; }; f'
    git remote add origin https://github.com/<username>/paperclip-customizations.git
fi
...
# One-time setup
cd $HOME/paperclip
git remote add myfork https://github.com/<username>/paperclip.git  # if not already added
...
- **Fork:** `<username>/paperclip` (private)
...
```bash
# 1. Clone backup
git clone https://github.com/<username>/openclaw-customizations.git /tmp/restore
cd /tmp/restore
...
### Paperclip Skills
```bash
git clone https://github.com/<username>/paperclip-customizations.git /tmp/pc-restore
...
git clone https://github.com/<username>/paperclip.git ~/paperclip
cd ~/paperclip
