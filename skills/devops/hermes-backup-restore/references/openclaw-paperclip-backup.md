...
if [ ! -d .git ]; then
    git init
    git config --local credential.helper '!f() { echo "username=<email>"; echo "password=$(gh auth token)"; }; f'
    git remote add origin https://github.com/<email>/openclaw-customizations.git
fi
...
if [ ! -d .git ]; then
    git init
    git config --local credential.helper '!f() { echo "username=<email>"; echo "password=$(gh auth token)"; }; f'
    git remote add origin https://github.com/<email>/paperclip-customizations.git
fi
...
# One-time setup
cd $HOME/paperclip
git remote add myfork https://github.com/<email>/paperclip.git  # if not already added
...
```bash
# 1. Clone backup
git clone https://github.com/<email>/openclaw-customizations.git /tmp/restore
cd /tmp/restore
...
### Paperclip Skills
```bash
git clone https://github.com/<email>/paperclip-customizations.git /tmp/pc-restore
...
git clone https://github.com/<email>/paperclip.git ~/paperclip
cd ~/paperclip
