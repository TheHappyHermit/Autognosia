...
minimal environment whose PATH lacks `${HOME}/.bun/bin` / global npm bin →
...
```bash
cat > ${HOME}/somewhere/tool-mcp.sh <<'EOF'
#!/bin/bash
exec "$HOME/.bun/bin/bun" "$HOME/.bun/bin/<tool>" serve --surface starter "$@"
EOF
chmod +x ${HOME}/somewhere/tool-mcp.sh
printf 'y\n' | hermes mcp add <name> --command ${HOME}/somewhere/tool-mcp.sh --connect-timeout 30
```
...
Real case: GBrain (`gbrain serve`) — wrapper at `${HOME}/.hermes/scripts/gbrain-mcp.sh`,
...
`${HOME}/.<app>/<app>-venv`, pip-install inside it, then `os.execv()` re-exec into
