...
deriver:
  ...
  healthcheck:
    test: ["CMD-SHELL", "pg_isready -U "$POSTGRES_USER" -d "$POSTGRES_DB""]  # Or just use database healthcheck
    interval: 30s
    timeout: 10s
    retries: 3
```
...
- `<email>/hermes-customizations` — Hermes skills, profiles, cron, vault
- `<email>/openclaw-customizations` — 19 skills, 8 agents, config, creds
- `<email>/paperclip-customizations` — 4 custom Paperclip skills
- `<email>/paperclip` — Full fork with Hermes adapter branch
