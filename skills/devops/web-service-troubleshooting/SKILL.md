...
### Container Running From a Stale/Temp Compose Path (verified 2026-08-25)

A service can be **up, healthy, and serving the wrong config** because it was
started from a throwaway clone. Symptom: a config change in the repo has no
effect no matter how many times you restart.

Always ask *where the running container was actually launched from*:
...
Real case: SearXNG was healthy for 42 hours while bind-mounting
`/tmp/<Repo>-clean/docker/core-config` into `/etc/searxng`. The repo's own
`settings.yml` was 1769 bytes and correctly enabled the JSON format; the `/tmp`
copy was a 205-byte stub that did not. Every config fix applied to the repo was
invisible to the container.

Two compounding hazards:

1. **`/tmp` is cleared on reboot.** The compose file named by the label had
   *already been deleted*, so `docker compose ... down` failed with
   `no such file or directory` — the stack could not be managed by compose at
   all. Remove such containers by name (`docker rm -f`) and re-create from the
   canonical path.
2. **A `/tmp` bind mount means config silently reverts** (or the container fails
   to start) after a reboot.

Scan every running container for this:

```bash
docker ps --format '{{.Names}}' | while read c; do
  cf=$(docker inspect "$c" --format '{{index .Config.Labels "com.docker.compose.project.config_files"}}' 2>/dev/null)
  case "$cf" in /tmp/*) echo "STALE: $c <- $cf" ;; esac
done
```

Note the distinction: a stale *label* alone only breaks compose management. A
stale *bind mount* actually breaks the service's configuration. Check both.

Before re-creating from the canonical repo, **carry over runtime secrets** the
temp deployment held. Check whether the repo compose expects an env var the
repo `.env` does not define — a placeholder like
`secret_key: "CHANGE_ME_IN_PRODUCTION"` in the tracked settings file is the
tell. Read the live value out of the old container first, write it to `.env`,
then recreate; otherwise sessions/preferences are invalidated.

### SearXNG Returns HTTP 403 On format=json

SearXNG's default `search.formats` is `[html]` only. Any programmatic client
requesting `&format=json` gets **403**, while the same query as HTML returns
200 — so the service looks perfectly healthy in a browser and to healthchecks.

Diagnose (loopback only, nothing leaves the host):

```
GET http://127.0.0.1:8080/search?q=test&format=json   -> 403 when unconfigured
GET http://127.0.0.1:8080/search?q=test               -> 200
docker exec <searxng> sh -c "grep -A4 'formats:' /etc/searxng/settings.yml"
```

Fix — in the mounted `settings.yml`:
...
Then recreate the container (a plain restart is not enough if the file is
mounted from the wrong place — see above).

**Why this matters:** an agent whose web backend 403s may silently fall back to
a different search provider and keep working, so the failure never surfaces as
an outage. Verify end-to-end by confirming the agent's own search tool stops
reporting a fallback/rescue — not merely that a raw request returns 200.
