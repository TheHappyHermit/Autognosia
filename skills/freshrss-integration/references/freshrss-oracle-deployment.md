# FreshRSS Deployment on Oracle Server (<oracle-server>)
...
2. **Deploy FreshRSS container with Traefik labels**:
   ...
   ssh -i ~/.ssh/oracle_cloud_key ubuntu@161.153.112.27 "docker run -d \
     ...
     -l 'traefik.http.routers.freshrss.rule=Host(\`freshrss.<oracle-server>\`)' \
...
3. **Verify deployment**:
   ...
   ssh -i ~/.ssh/oracle_cloud_key ubuntu@161.153.112.27 "curl -s -H 'Host: freshrss.<oracle-server>' http://localhost/api/greader.php"
   ```

4. **Complete initial setup** (browser-based):
   - Navigate to `http://freshrss.<oracle-server>/install.php`
...
- Traefik automatically routes `freshrss.<oracle-server>` to the container via Docker labels
