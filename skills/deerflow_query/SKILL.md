...
## Base URLs
- Base URL: https://deerflow.[private-site].com
- Gateway URL: https://deerflow.[private-site].com
...
### Health Check
```bash
curl -X GET https://deerflow.[private-site].com/health
```

### Create Thread
```bash
curl -X POST https://deerflow.[private-site].com/api/threads \
...
```bash
curl -X POST https://deerflow.[private-site].com/api/threads/{thread_id}/runs/stream \
  -H "Content-Type: application/json" \
  -d '{
