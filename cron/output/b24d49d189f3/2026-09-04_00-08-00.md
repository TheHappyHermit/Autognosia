...
## Frontier Research Round 27 — Completed

**File created:** `research/frontier-research-ontology-constrained-decoding-epistemic-paradox-intermediate-languages-2026-09-04.md`
- **Size:** 27,382 bytes (321 lines)
- **Status:** Written, verified, synced to brain DB (21 chunks)

**Backlinks added:** 12 wikilink backlinks to related research files in the vault (Rounds 23–26, dual-memory, temporal-phenomenology, grounding-hallucination, causal-reasoning, etc.)

**Topics covered (7 papers/ideas across 5 domains):**

1. **Graph-Constrained Reasoning (GCR)** — Trie-based decoding-level KG grounding achieving 0% hallucination; ontology as generative grammar rather than retrieval target
2. **OntoLLM** — Ontology-constrained prompting reducing LLM digression by 30%; ontology-as-constraint at prompt level
3. **Intermediate Language Challenge** — Formal language choice (FOL vs ASP vs Pyke) as first-class design variable in neurosymbolic reasoning; FOL outperforms
4. **Epistemic Coherence Paradox** — Bidirectional dissociation of competence and explanation; the basing-relation gap for AI understanding; implications for dual-memory architecture
5. **Step-Level KG Grounding** — GRBench results showing 26.5% improvement; provenance trails for every reasoning step
6. **BFO Change Representation** — Formal mechanisms for representing continuant change beyond "history" type; convergence with WorldDB's edge types
7. **My Ontologist** — LLM-assisted BFO ontology engineering evaluation; hybrid human-in-the-loop verification

**Brain sync:** Successfully synced — 21 chunks indexed in PostgreSQL (ollama embedding waited 57s before completing).
