...
- One line per topic area: "Topic: X papers found, Y new insights"
- File created: path, size, chunks synced
- DO NOT dump full paper summaries or skill content
- DO NOT include verbose stats or timing details

## Response

## Frontier Research Report — 2026-09-02

**Research scope:** 7 topic areas (ontology engineering, philosophy ontology, psychological ontology, ontology alignment, AI ontology failures, LLM + structured knowledge, dual memory, dynamic ontology, experiential memory) | **~60 sources** extracted from web research

### Topic Summary

- **Ontology engineering:** 3 frameworks identified (OntoQA, WiseOWL, EAPB); LLM-assisted and modular (MOMo) paradigms emerging; 8 papers found, 3 new insights (LLM-scaffolded ontology engineering, WiseOWL zero-data evaluation, MOMo voting-based pruning)
- **Philosophy ontology:** BFO now ISO-standard + 2024 DoD adoption; DOLCE superior for cognitive domains; ontology governance replacing single-TLO selection; 6 papers, 3 insights (institutional standardization shift, governance paradigm, TLO trade-off analysis)
- **Psychological ontology:** CoTOn/MemOnto formalize cognitive concepts; cognitive ontology debate (top-down vs bottom-up); mental representation classicalism vs connectionism vs 4E non-representationalism; 8 papers, 4 insights (theory-dependent conceptualization formalization, cognitive ontology pluralism, representation debate evolution)
- **Ontology alignment:** Automated ITSMO→DOLCE mapping 73.5% accuracy with LLMs; EMMO highest richness but query cost; pairwise mapping combinatorial bottleneck; ODPs as scalable alignment; 5 papers, 3 insights (AI-assisted alignment viability, semantic richness vs efficiency trade-off)
- **AI ontology failures:** ChatGPT's implicit ontology diverges from BFO (abstract→concrete vs continuant→occurrent); semantic entropy detects knowledge-gap hallucinations; HALO ontology first formal hallucination taxonomy; KG guardrails reduce hallucination; 7 papers, 5 insights (LLMs reproduce ambiguity not resolve it, semantic entropy as detection method, KG as reasoning guardrail)
- **LLM + structured knowledge:** Neuro-symbolic patterns (KEA Explain graph kernels, NCAI with OPM, NeuSymMS with CLIPS); neural for perception, symbolic for reasoning/memory; Evo-Memory test-time learning; 6 papers, 4 insights (neuro-symbolic paradigm crystallizing, graph kernel hallucination explanation, OPM for process/state modeling)
- **Dual memory:** D-Mem dual-process (System 1 fast / System 2 deliberate) with metacognitive gating; PRIME uncertainty-triggered deliberation; MemoRAG global memory for clue generation; 5 papers, 3 insights (dual-process applied to memory management, not just reasoning, uncertainty gating is key differentiator)
- **Dynamic ontology:** Runtime evolution (new concepts vs enrichment), CDC domain-contextualized graphs with 20+ core relations, KERAIA dynamic relations inspired by Minsky K-lines, EANO actor-network reflexive structures; 5 papers, 4 insights (ontology as continuously evolving system, traceable provenance for conceptual change, Minsky's K-lines as formal LoT construct)
- **Experiential memory:** EMS 7 primitives (temporal anchoring, experiential content, affective valence, relevance gradient, contextual embedding, consolidation state, decay trajectory); episodic memory as mental model reconstruction not fact retrieval; scenario construction framework; 7 papers, 5 insights (no existing AI dataset achieves EMS Level 1 compliance, episodic ≠ timestamped retrieval, trace minimalism — EM traces store pointers not content)

### New File Created

- **Path:** `~/.autognosia/active-wiki/research/frontier-research-ontology-psi-memory-sept2026.md`
- **Size:** 19,426 bytes (388 lines)
- **Cross-references:** Supplements existing research (round13–round20 files, commonsense, knowledge-structures)
- **Chunk sync:** `brain_sync.py` not found at expected path — sync step skipped

### Cross-Cutting Insights for Autognosia

1. BFO is now the institutional standard (2024 DoD adoption) but DOLCE may be superior for cognitive/epistemic domains
2. Ontology engineering is going modular + LLM-assisted (expert-authored → LLM-scaffolded, expert-verified)
3. Neuro-symbolic integration is the dominant architecture: neural for perception, symbolic for reasoning/memory
4. Dynamic ontology (runtime evolution with observability) is the next frontier — static schemas are insufficient
5. Episodic memory is a temporal reconstruction problem, not fact retrieval — vector search cannot capture EMS primitives
