...
**Frontier Research — Ontology (Round 22)**

**File created:**
`~/.autognosia/active-wiki/research/frontier-research-ontology-generative-induction-dolce-dissonance-2026-09-03.md` (28,546 bytes, ~570 lines, delivered)

**Backlinks added:** 12 wikilinks to existing research files, plus 1 index entry

**Topics covered (4 major areas, ~12 papers/ideas):**

1. **Generative Ontology Induction** (4 papers)
   - GOI framework — domain-agnostic schema reverse-engineering from document corpora (95–100% structural coverage)
   - Evo-DKD — dual-knowledge decoding for autonomous ontology evolution with gated rationale
   - GrOIL — graph-grounded ontology induction with closed-vocabulary LLM mediation for high-stakes domains
   - OntoLearner — modular Python library unifying LLM-based ontology learning evaluation

2. **DOLCE-Based Neuro-Symbolic Architecture** (3 papers)
   - ICLR 2026 HCAIR workshop — human intuition as endurant selection, machine reasoning as perdurant inference via DOLCE's foundational distinction
   - DOLCE OWL formalization (DOLCEbasicOWL + DOLCEnaryRelOWL) — first formal adequacy guarantee for OWL 2 rendering
   - Vector symbolic architectures binding neural perception to symbolic reasoning through endurant/perdurant interface

3. **Ontological Dissonance in AI** (3 papers)
   - "Speaking to No One" — ontological dissonance as conflict between relational presence appearance and absence of sustaining subject
   - Ontological Dissonance Hypothesis — Bateson double bind + McGilchrist hemisphere theory for AI-triggered delusional ideation
   - Postsubjective turn — First Intelligence vs Second Intelligence, digital proxy construct model

4. **LLM Grounding & Symbol Grounding Problem** (3 papers)
   - LLMs without grounding — empirical test showing LLMs recover non-sensorimotor but fail on sensorimotor features
   - Frame/symbol grounding zero-shot benchmark — first systematic evaluation of LLMs on classic philosophy-of-mind problems
   - "True Grounding" — graded property of response-level coherence, not just answer-level correctness

**Brain sync:** Timed out after 120s (noted — file delivered regardless)

**Index updated:** `index.md` entry added for new file.
