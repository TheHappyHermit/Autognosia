...
## Frontier Research Round 18 — Complete

Successfully researched all 10 mandated topics and wrote the report to `~/.autognosia/active-wiki/research/frontier-research-ontology-knowledge-structures-sept-2026-round18.md` (26.6KB).

### What's New Since Round 17

The major discovery is the emergence of **ontology-as-a-kernel** as the new default architecture:

- **OaK (arXiv 2608.22974)** — Ontologies are no longer retrieval indexes but executable interfaces mediating between LLM agents and domain knowledge. Schema + function catalog pattern now the reference architecture
- **CoEvoKG (arXiv 2608.13662)** — Self-evolving knowledge graphs where verified evidence is written back to the graph, creating a closed loop between model evolution and knowledge accumulation (+11.2% accuracy improvement)
- **DIAL-KG (arXiv 2603.20059)** — Schema-free incremental construction with governance-centric closed loops
- **Self-Evolving Agents as Dynamic Graph Transformation (arXiv 2608.18104)** — Formal framework mapping 46 existing methods into a unified graph rewrite theory
- **AVA (arXiv 2609.00177)** — Benchmark revealing embeddings CANNOT capture ontological reasoning (171K contrastive triplets across 163 ontologies)

### Cross-Cutting Synthesis

1. **Ontology-as-Kernel** — Ontologies as executable interfaces, not passive indexes
2. **Self-Evolving KGs** — Continuous enrichment through evidence writeback (static "build once" is obsolete)
3. **Dynamic Graph Transformation** — Unifying formal theory for all evolution patterns
4. **Embeddings Can't Replace Ontology** — Persistent gap between linguistic and ontological discrimination
5. **Neurosymbolic as Production Standard** — FAOS v4: 650+ agents across 22 industry verticals

### 24 sources cited with full inline citations and Sources block

### brain_sync Issue

`brain_sync.py` failed because `pg8000` module is not installed. This is a dependency issue, not a research failure. The file is on disk and ready for indexing when the environment is fixed.
