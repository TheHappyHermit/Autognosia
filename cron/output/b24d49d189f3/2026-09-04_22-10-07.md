...
brain_sync completed successfully — 1 new file, 1 updated (index), 28 chunks embedded, 0 errors.

---

**Output delivered.**

**File created:** `~/.autognosia/active-wiki/research/frontier-research-ontology-round44-2026-09-10.md`
- **Size:** 35,228 bytes / 352 lines
- **Status:** Written, index updated, brain_sync synced (28 chunks, 0 errors)

**Backlinks added:** 42+ [[wikilinks]] to related research files across Rounds 35–43 and cross-references to key concepts (Knowledge-Boundary Law, MASEO, ECS, Constitutional Memory, etc.)

**Topics covered (8 papers/ideas):**

1. **Ontological Continuum** (Daga/Tamma/Payne, arXiv 2605.22093) — 4D KG characterization framework: semantics×pragmatics, properties×affordances. 10 KGs mapped empirically via FCA.
2. **Experience Compression Spectrum** (Zhang et al., arXiv 2604.15877) — Memory/skills/rules unified on single compression axis (1:1 → 10× → 100× → 1000×+). "Missing diagonal" identified: no system supports adaptive cross-level compression. <1% cross-citation between memory and skill communities.
3. **OntoLearner Benchmark** (arXiv 2607.01977) — 180 ontologies, 22 domains. Central finding: failure modes scale with ontological complexity, not model size.
4. **LLM Scale Effects on OL** (arXiv 2608.31118) — 13 models controlled study. Dense 27B > sparse 70B on term typing. Architecture > parameter count.
5. **Memory as Ontology / Animesis CMA** (Li, arXiv 2603.04740) — Four-layer governance hierarchy (governance/epistemic/experiential/procedural). Governance before functionality. Identity continuity across model transitions.
6. **Recuris Recursive Memory** (Yu et al., arXiv 2608.24876) — Evidence-localized memory updates. Working Memory (task state) + Experiential Memory (skill pool) + Meta-Agent (evidence → skill updates). Bounded recursive loop.
7. **Open Ontologies MCP** (arXiv 2605.09184) — Rust + MCP + stable matching alignment. F1=0.832 on OAEI Anatomy. MCP tools (F1=0.717) >> raw OWL file reading (F1=0.323).
8. **LLMs4OL 2026 Challenge** (arXiv 2608.27101) — RAG + vocabulary-constrained filtering. F1=0.92 term typing, 0.85 taxonomy discovery. Qwen2.5-14B with good retrieval > larger unassisted models.

**Cross-cutting themes:** Ontological Continuum as meta-theory; compression as central long-horizon agent problem; ontological complexity > model size; tool-augmented access > raw text; governance before functionality; missing diagonal as Autognosia opportunity.
