...
**Overdue / Due Today**
- 🔴 **Weekly System Review** — due today (Aug 24). Priority: medium.

**Upcoming Tasks**
- ⬜ Research and select a project name — due Sep 1
- ⬜ Review subscription billing before Sep 16 — due Sep 9

**Active Projects**
- Hermes Cortex Setup

**Subscriptions (next 30 days)**
- Example Subscription — $9.99 USD/month, next billing Sep 16
...
- **Active Wiki**: No files modified in the last 24 hours.
- **Oracle Brain**: No files modified in the last 24 hours.
- No overnight research or wiki changes to review.
...
| Check | Status |
|-------|--------|
| **Disk** | 46% used (68G / 158G) — 83G free ✅ |
| **Docker** | 11/12 containers healthy ⚠️ |
| **GBrain** | Sync OK (803 pages) ⚠️ |
| **Cron** | No cron.log found |

**Alerts:**
1. 🔴 **firecrawl-api** — container is in a restart loop (status: `Restarting (1) 40s ago`). Needs investigation.
2. 🟡 **GBrain upgrade available** — 0.46.11 → 0.46.28. Run `gbrain self-upgrade`.
3. 🟡 **GBrain autopilot not installed** — no cycles have ever run (full or targeted).

---

## Recommended Actions

1. **Complete today's Weekly System Review** task — overdue.
2. **Investigate firecrawl-api restart loop** — `docker logs docker-firecrawl-api-1` to check the failure reason.
3. **Run `gbrain self-upgrade`** when convenient — 3 minor versions behind.
