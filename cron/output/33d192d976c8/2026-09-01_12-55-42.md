...
**Research Output: Frontier Research — KG Construction, Ontology & Memory (Late September 2026)**

**New file written:** `~/.autognosia/active-wiki/research/frontier-research-kg-ontology-sept-late-2026-final.md` (18,952 bytes)

**12 sources registered** in the citation ledger with full sourced citations.

---

**12 new findings discovered across 9 topics:**

**Knowledge Graph Construction (RAGA, Nexarag):** Autonomous KG construction has reached production-ready status via RAGA's Read-Search-Verify-Construct loop and Nexarag's MCP-native platform — no more batch dedup or blind insertion. Nexarag is a production-ready template for exposing Autognosia's semantic memory via MCP.

**Tree-of-Experience (ToE):** Organizing experience by reasoning structure (not task similarity) yields +20.4pp accuracy on Game of 24 with -78.7% LLM calls. Critically: poorly organized experience actively hurts performance — the "bad memory hurts" principle is now empirically validated.

**ExpWeaver (Latent RAG):** Encodes experiences in the LLM's own hidden states for on-policy alignment, retrieving in latent space to bypass context window limits. SOTA on 12/13 tasks. Rethinking Experience Utilization confirms selective interweaving is the optimal usage pattern.

**NextKB:** Northwestern's open-license KB integrating OpenCyc + FrameNet + WordNet + VerbNet — the closest existing realization of the "Cyc successor" vision.

**KGHaluBench:** Dynamic KG-based hallucination benchmark — ~39.74% of proficient model responses contain at least one hallucinated fact. Methodology applicable to Autognosia's KG quality assurance.

**Context Graph vs Ontology:** The field has converged on a five-term taxonomy (Taxonomy → Ontology → KG → Semantic Layer → Context Graph). Ontology is a sub-component of context graph — validates Autognosia's Oracle Wiki (ontology) + Active Wiki (context graph) architecture.

**Semantic Backbone:** GraphWise proposes SKOS → OWL → KG → Context Graph as the production architecture for enterprise agentic AI.

**FrameNet SRL by Analogy:** LREC 2026 demonstrates analogical transfer over FrameNet can achieve SOTA semantic role labeling without explicit role supervision — a frugal approach for document understanding.

**Autonomous KG Construction:** The 2026 convergence is on LLM agents with CRUD surfaces grounded in provenance — RAGA, TRACE-KG, OntoKG, SHARP, HyperGraphPro all follow the same pattern.

---

**brain_sync:** Ran but **timed out** — PostgreSQL (127.0.0.1:5433) is reachable, but **Ollama (11434) is down**, which brain_sync requires for embedding. The script hung on the Ollama connection attempt. Brain sync needs Ollama running at 11434 to complete.
