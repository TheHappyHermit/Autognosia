...
I've completed the research and written the findings file. Here's the summary:

## Frontier Research Round 13 — Completed

**File written:** `research/frontier-research-knowledge-structures-sept-2026-round13.md`
- **Size:** 24,178 bytes / 340 lines
- **Sources:** 13 new sources registered in citation ledger

**What's new vs. Round 12:**

1. **Knowledge Graph Construction** — Hybrid three-layer architecture (vector→graph→ranking) has crystallized; skeleton indexing confirmed as production standard
2. **Semantic Web** — Confirmed RDF 1.2/SPARQL 1.2 stuck at WD status for 6+ months; advises using RDF 1.1/SPARQL 1.1 as safe baseline
3. **Taxonomy vs Ontology** — Industry consensus settled: Neo4j's framework (taxonomy=classification, ontology=meaning, KG=instance data) and iBL.ai's three-level stack confirm Active Wiki's structure is correct
4. **Ontology Evaluation** — Three new developments: WiseOWL (quantitative internal scoring), semantic preservation metric (new dimension beyond structural correctness), LLM scale study (9B→27B gains plateau for dense models)
5. **Cyc/WordNet/FrameNet** — FrameNet at 25 milestone; NextKB integration lesson (complementary resources > competing approaches)
6. **Dual Memory** — Four new architectures: SEA (RL joint-optimization), HeLa-Mem (Hebbian learning), CraniMem (gated encoding), long-horizon scientific agent (15K messages, 70-85% accuracy)
7. **Dynamic Ontology** — Evo-DKD: continuous autonomous evolution with dual-decoder closed loop, beyond OaK's construct-then-freeze
8. **Experiential Memory** — DeltaMem (residual trees), ExpeL (cross-task abstraction without parameter updates)

**brain_sync.py** timed out (300s limit, likely processing the full active-wiki). The file is written and ready for indexing on the next sync cycle.
