...
**Research Round 37 — Delivered**

**File created:** `/home/josh434/.autognosia/active-wiki/research/frontier-research-ontology-round37-2026-09-07.md`
- **Size:** 43,929 bytes (414 lines) — 9 topics across ontology engineering, memory governance, and embedding limitations
- **Status:** Complete, brain_synced to vector search (29 chunks, 0 errors)

**Papers analyzed (9):**

| # | Paper | Core Contribution |
|---|---|---|
| 1 | **AVA** (Giglou et al., CIKM 2026) | Embeddings achieve only 0.135 accuracy on hard negative ontological discrimination — proves embedding-only approaches cannot replace symbolic reasoning |
| 2 | **OntoLearner** (Giglou et al., 2026) | Infrastructure layer: 180 ontologies, 22 domains, 22 retrieval models, 12 LLMs standardized for ontology learning evaluation |
| 3 | **Synapse** (Jiang et al., ACL Findings 2026) | Spreading activation memory — cognitive science-inspired dynamic graph retrieval bridging episodic and semantic memory |
| 4 | **SSGM** (Lam et al.) | Stability & Safety-Governed Memory — formal drift bound O(N·ε_step), governance middleware decoupling memory evolution from execution |
| 5 | **VirtualSet** (Zhang, 2026) | Typed ontology world interface — LLMs emit type-checked set expressions instead of SQL; +4.0 points on BIRD with pre-execution error catching |
| 6 | **gUFO** (Almeida et al., 2026) | Gentle Foundational Ontology — ISO-standardized UFO implementation with relator entities, situation patterns, and type typology in OWL 2 DL |
| 7 | **Liquid Interfaces** (2026) | Ephemeral coordination ontologies — interfaces as dynamically constructed, per-interaction ontologies rather than persistent schemas |
| 8 | **D-Mem** (2026) | Dual-process memory — fast retrieval + slow deliberation with multi-dimensional quality gating for process selection |
| 9 | **Context Cartography** (Wu & Gartner) | Cartographic operators for context governance — projection, zoom, reprojection, boundary management as structural compensation for LLM context limitations |

**Backlinks added:** 15 unique links to other research files across Rounds 31-36, plus foundational papers on neuro-symbolic reasoning, dual memory, hallucination, and grounding.

**Topics covered:** Ontology engineering (5 papers), philosophy ontology/foundational ontologies (2 papers), psychological ontology/memory governance (2 papers), ontology alignment (1 paper), BFO/SUMO/DOLCE/gUFO (1 paper), AI ontology failures/grounding (2 papers), LLMs + structured knowledge (3 papers), Dual Memory (3 papers), Dynamic Ontology (2 papers), Experiential Memory (2 papers).

**brain_sync:** Completed successfully — 224 files scanned, 1 updated, 29 chunks indexed, 0 errors. Duration: ~100s (Ollama was busy, waited up to 60s between batches).

**Key new insights for Round 37:**
- AVA's 0.135 hard negative accuracy is the strongest evidence yet that embedding-based methods cannot capture ontological reasoning — validates the symbolic grounding thesis
- SSGM proves governance can bound semantic drift independently of system lifetime — a formal guarantee for long-running agents
- Liquid Interfaces proposes ephemeral per-interaction ontologies, the most radical departure from persistent schema engineering
- Context Cartography provides the formal mathematical vocabulary for context management as structured governance
