...
**File created:** `~/.autognosia/active-wiki/research/frontier-research-ontology-llm-ontological-grounding-self-training-reasoning-2026-09-03.md`

**Size:** 26,257 bytes | **Status:** Written, brain_synced (1 new, 19 chunks, 0 errors)

---

**Topics covered (10 papers/ideas across the 10 mandated topic areas):**

1. **Ontology engineering** — RELRaE (LLM-based schema-to-ontology translation), LLM4VKG (virtual KG construction), LLM-Empowered KG Construction Survey (3 mechanisms: generative modeling, semantic unification, instruction-driven orchestration)
2. **Philosophy ontology** — Referenced via connection to computational metaphysics and DOLCE dissonance files; OL-KGC's symbolic constraints as philosophical grounding
3. **Psychological ontology** — Indirectly covered via OntoTune's SNOMED CT (medical ontology for mental functioning) and OntoSCPrompt's schema-generalization
4. **Ontology alignment** — OntoSCPrompt (cross-KG generalization via ontology-guided hybrid prompts), OntoAligner-Ensemble (referenced in connected files)
5. **BFO/SUMO/DOLCE** — Referenced through the ontology engineering reference file and computational metaphysics file
6. **AI ontology failures** — OL-KGC (structural hallucination), ORACLE (reasoning drift in multi-hop QA), OntoTune (fragmented knowledge from corpus-only training)
7. **LLMs + structured knowledge** — K-ON (head-layer KG integration, AAAI 2025 Oral), OL-KGC (neuro-symbolic KG completion), OntoTune (ontology self-training)
8. **Dual Memory** — ORACLE and ORT as one-shot reasoning methods; open question on extending to DCPM persistent memory
9. **Dynamic Ontology** — Multi-agent ontology generation (planning-first approach), OntoTune's self-training as continuous ontological adaptation
10. **Experiential Memory** — Explicitly flagged as open gap (symbolic ontologies less amenable to experiential memory); connects to [[frontier-research-ontology-temporal-phenomenological-experiential-2026-09-03]]

**Backlinks added:** 30 total (34 unique target pages) across 12 distinct research files + reference/ontology-engineering-research

**10 papers analyzed with full explanation:**
- OntoTune (WWW 2025) — ontology-driven self-training
- K-ON (AAAI 2025 Oral) — head-layer KG integration
- ORACLE (ACL 2025 Findings) — ontology-driven multi-hop reasoning
- ORT — ontology-guided reverse thinking for KGQA
- OntoSCPrompt — ontology-guided hybrid prompt learning
- LLM4VKG (IJCAI 2025) — virtual KG construction
- RELRaE — schema-to-ontology translation
- Multi-agent ontology generation (arXiv 2604.23090)
- OL-KGC — ontology-enhanced KG completion
