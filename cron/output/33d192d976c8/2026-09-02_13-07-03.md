...
The brain_sync requires pg8000, Ollama, and Postgres — infrastructure dependencies that need to be running. The file is written and verified; I'll report findings.

---

**Research Complete — Frontier Research: Ontology Evaluation, Cyc/WordNet/FrameNet Lessons, Neuro-Symbolic AI, GraphRAG**

**File written:** `~/.autognosia/active-wiki/research/frontier-research-ontology-evaluation-legacy-neuro-symbolic-2026-09-02.md` (404 lines, ~24KB)

**brain_sync:** Requires `pg8000`, Ollama, and Postgres — infrastructure dependencies not currently available in this environment. The file is ready for sync when these services are running (`python3 scripts/brain_sync.py --source active-wiki`).

### What This Report Covers (Gaps Filled vs. Previous Research)

| Topic | What New |
|-------|----------|
| **Ontology Evaluation** | WiseOWL (4-metric scoring), EAPB (entropy-based redundancy), OntoQA, Jaziri & Sassi ML framework — none of these were in prior reports |
| **Cyc Project** | Full architecture (CycL, microtheories, inference engine), 40-year history, lessons: explicit knowledge beats statistical approximation, but manual encoding cost killed scalability |
| **WordNet/FrameNet/VerbNet/PropBank** | Lexical knowledge base design patterns — synset clustering, frame-based event modeling, subevent tracking, predicate-argument structure |
| **Neuro-Symbolic AI** | Three integration patterns (KG as external memory, KG from LLM output, bidirectional), robustness/UQ/intervenability dimensions |
| **GraphRAG** | Graph traversal for relational retrieval vs. vector-only RAG, enterprise ROI data, multi-layer hybrid retrieval |
| **Test-Time Learning** | Evo-Memory benchmark, ReMem pipeline, ERL heuristic distillation, MemoPilot trainable memory updates |

### Key Insights for Autognosia

1. **Ontology quality should be automated at write-time** — WiseOWL-style scoring + entropy-based redundancy detection
2. **Active Wiki is naturally neuro-symbolic** — structured frontmatter/metadata = symbolic layer, LLM search/summarization = neural layer
3. **FCA lattices can serve as the symbolic constraint layer** for LLM-generated content validation
4. **SPARQL endpoint for Active Wiki** would enable structured queries over wiki structure
5. **Heuristic distillation** (not raw trajectory replay) is the dominant pattern for agent self-improvement
6. **Legacy lexical resources** (WordNet synsets, FrameNet frames, VerbNet subevents) provide principled patterns for conceptual clustering and event modeling

### Sources: 40 registered in citation ledger

Ledger at `~/.hermes/cache/citations/ledger.json` with `[1]` through `[40]` mapped to all cited URLs.
