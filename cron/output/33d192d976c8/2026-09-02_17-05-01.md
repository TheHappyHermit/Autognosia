...
brain_sync.py requires pg8000 + PostgreSQL which are unavailable (no pip, no apt/sudo). The file is written and verified on disk — sync will pick up on the next cron run when the environment has the deps available.

Here's the summary:

**Topic: 10 topic areas — comprehensive coverage across all mandated subjects**

**New findings:**
- **OaK (Ontology-as-Kernel)**: Dynamic task-conditioned ontology kernel (S + F) for LLM agents — schema bounds expression, functions bound computation. Evaluated on TravelPlanner/CRMArenaPro/ToolQA with consistent improvements.
- **SYNAPSE**: Spreading-activation memory architecture achieving 95% token reduction with +7.2 F1 on LoCoMo — cognitive science principles measurably outperform vector similarity.
- **DGMM**: Memory-centric architecture where propositions are generated at recall time, not stored — additive memory invariant.
- **NEST**: Graph-theoretic ontology for cognition with dual durable belief / capacity-limited working-memory graphs.
- **MNEMA**: Agent = persistent memory system; LLM = invoked service. Challenges file-based identity patterns.
- **Structural Hallucination**: Graph-level failure invisible to sentence-level metrics (0.028 Jaccard similarity in Roget reconstruction).
- **Ontology Alignment**: Affordance-based (not structure-based) view of cross-KG integration emerging.
- **Upper Ontologies**: BFO/CCO adopted by US DoD/IC (largest endorsement in history); gist 14.0.0 with new enterprise classes; SULO lightweight alternative.

**File created:**
- `~/.autognosia/active-wiki/research/frontier-research-ontology-dual-memory-dynamic-ontology-2026-09-02.md` — 21,208 bytes
- **brain_sync:** FAILED — pg8000 module unavailable (no pip/apt in current environment). Will sync on next run when deps are present.
