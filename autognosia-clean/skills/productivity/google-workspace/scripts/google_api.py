...
# Read-only gate: reject any write operations at parse time
_READ_ONLY_ACTIONS = {
...
def _enforce_readonly(service, action):
    allowed = _READ_ONLY_ACTIONS.get(service)
    if allowed is None or action not in allowed:
        print(f"BLOCKED: '{service} {action}' requires write access. Read-only mode enforced.", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    _enforce_readonly(args.service, args.action)
    args.func(args)
