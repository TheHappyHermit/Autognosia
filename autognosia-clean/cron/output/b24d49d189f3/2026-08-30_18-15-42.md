...
The file has been created successfully (~3,284 words, 26KB) at `/home/josh434/.autognosia/oracle/brain/Memory-Theory/Memory-Trace-Decay-vs-Interference.md` and brain_sync is running in the background to index it. The report covers:

- **Historical foundations**: Ebbinghaus (1885) → Thorndike (1914) decay → McGeoch (1932) interference challenge → Underwood (1957) proactive interference
- **Core debate**: Decay vs interference mechanisms, key empirical dissociations (Brown-Peterson, Jenkins-Dallenbach, Waugh-Norman, Lewandowsky distractor variability)
- **Computational models**: SIMPLE (temporal distinctiveness, scale-invariant), SOB/SOB-CS (interference via superposition), TBRS*-I (hybrid decay + interference, twofold refreshing), Georgiou et al. (valence-based retroactive erasure, power-law)
- **Neurobiological evidence**: Synaptic vs systems consolidation, reconsolidation, sleep-dependent reactivation, prefrontal interference resolution
- **Autognosia mapping**: Gaps in current memory systems (no automatic forgetting, no consolidation pipeline, no interference detection, no reconsolidation), design patterns from each model
- **System-specific overlaps/tensions**: Active Wiki, Brain Search, Honcho, Oracle Brain, Graphify
