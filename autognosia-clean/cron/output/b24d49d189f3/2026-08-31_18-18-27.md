...
The Graph RAG research report has been written to `/home/josh434/.autognosia/oracle/brain/Memory-Architecture/Graph-RAG-Graph-Retrieval-Augmented.md` (22,619 bytes, 290 lines). The brain sync is running in the background.

The report covers:
- **Core GraphRAG pipeline**: LLM-driven entity extraction → Leiden community detection → bottom-up summarization → map-reduce global search
- **LightRAG alternative**: dual-level retrieval (low-level entities + high-level themes) with incremental graph updates (key advantage: ~50 tokens per new doc vs ~14M for GraphRAG full rebuild)
- **Multi-hop reasoning**: MultiHop-RAG benchmark (2,556 queries, 4 types: Inference/Comparison/Temporal/Null), 2WikiMultiHopQA, HotpotQA — GraphRAG wins on multi-hop, loses on single-hop
- **When to use graphs**: Xiang et al.'s decision tree — graphs pay for cross-document synthesis, aggregation queries, multi-hop; vector wins for single-chunk fact lookup
- **Criticisms**: evaluation bias (order/length/trial), indexing cost ($33K→$33 for 5GB but still 10× vector RAG), entity extraction errors propagate structurally, fragmented graphs
- **Autognosia mapping**: Brain Search (add Leiden on wikilink graph), Honcho (entity graph with temporal/causal edges), Active Wiki (community summaries on existing wikilinks), Oracle Brain (entity graphs per domain), Graphify (page-level vs entity-level complement), multi-memory cross-synthesis
