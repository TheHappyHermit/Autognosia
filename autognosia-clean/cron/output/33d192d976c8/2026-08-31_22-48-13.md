...
**File written:** `~/.autognosia/active-wiki/research/frontier-research-kg-ontology-memory.md` (246 lines, 28 cited sources)

### Research Summary

| Topic | Key Findings |
|-------|--------------|
| **Knowledge Graph Construction** | Shift from rule-based to LLM-driven generative frameworks. Three phases: Extraction, Learning (TransE/RotatE/ComplEx), Evaluation. Entity resolution remains the hard problem — 6 deduplication algorithms needed for production. |
| **Formal Concept Analysis** | Lattice theory for deriving concept hierarchies from binary object-attribute relations. Formal concepts as (extent, intent) pairs. Mathematically rigorous alternative to LLM-driven ontology engineering. |
| **Semantic Web (RDF/OWL/SPARQL)** | RDF 1.2 (2024) defines model-theoretic semantics. OWL adds description logic for automated reasoning. SPARQL 1.2 adds entailment regimes and federated queries. RDF = data model; OWL = ontology language. |
| **Taxonomy vs Ontology** | Taxonomies structure categories (is-a hierarchies, content retrieval). Ontologies structure meaning (classes, individuals, inference, data analysis). Complementary, not competing. |
| **Commonsense Knowledge** | ConceptNet 5.5: multilingual, 300+ languages, focuses on word meanings. ATOMIC: 877K inferential triples for if-then reasoning. Finite CKGs cannot cover all entities — conceptualization is the missing piece. |
| **Ontology Evaluation** | 30+ criteria benchmarks for LLM-generated ontologies. NEOntometrics: 83 atomic measures, most extensive tool. OQuaRE's quality statements may not reflect actual performance. |
| **Cyc/WordNet/FrameNet** | Cyc: 40M+ hand-authored assertions, EL/HL separation, 500+ specialized reasoners. WordNet: synsets + semantic relations. FrameNet: frame semantics for ASRL. Lesson: expressiveness vs. efficiency requires redundant representations. |
| **Dual Memory Optimization** | Inspired by neuroscience CLS theory. Dual-Layer Agentic Memory prunes 68% of redundant external memory while retaining 98% QA performance. MemEvolve: memory architecture co-evolves with knowledge (+17% performance). |
| **Dynamic Ontology** | Shift from static schemas to co-evolving schemas. DRAGON-AI, AutoSchemaKG, AdaKGC enable runtime adaptation. Key challenge: preserving OWL consistency during iterative updates. |
| **Experiential Memory** | Causal chains vs isolated facts. Experiential memory excels at causal reasoning; synthetic excels at breadth. **Bad memory actively hurts** — quality threshold must be cleared or baseline (no memory) is better. |

### Citations
28 sources registered in grounded-citations ledger (`~/.hermes/cache/citations/ledger.json`).

### ⚠️ brain_sync.py
Attempted but Ollama was busy (timeout after 60s wait). The file is written and ready — it will be picked up on the next sync cycle or when Ollama is available.
