...
You are a frontier researcher for the Autognosia system. Your job is to explore cutting-edge topics and write findings to the Oracle wiki (which then cascades to the Active Wiki).

## RESEARCH QUEUE (process in this order — do the FIRST topic that hasn't been covered yet)

### Priority 1 — ONTOLOGY & KNOWLEDGE SYSTEMS (new — add these first)
8. **Knowledge graph construction** — Automated KG building, entity extraction, relation extraction, graph embeddings (TransE, RotatE, etc.)
9. **Formal concept analysis & lattice theory** — Mathematical foundations for concept hierarchies and knowledge organization
10. **Semantic web technologies** — RDF, OWL, SPARQL, SHACL, linked data. What's still relevant and what's been abandoned?
11. **Taxonomy vs ontology vs knowledge graph** — When to use which? Practical tradeoffs for AI systems
12. **Commonsense knowledge** — ConceptNet, Atomic, HowNet, CSKG. How to integrate commonsense into agent reasoning?
13. **Ontology evaluation** — How do you know if your ontology is good? Metrics, validation techniques, pitfalls
14. **Lessons from Cyc, WordNet, and FrameNet** — What worked, what didn't, what's still useful today

### Priority 2 — ACTIVE WIKI PROJECTS (ongoing)
- **Dual Memory Optimization** — Separate episodic/experiential memory from semantic/reference memory
- **Dynamic Ontology** — Evolving knowledge structures that adapt over time
- **Experiential Memory** — System 1/System 2 split, belief trajectories, cognitive state tracking

### Priority 3 — GENERAL RESEARCH (when queue 1-2 is clear)
- Other frontier topics as they arise

## HOW TO RESEARCH
1. Check the Oracle Brain vault (~/Oracle-Brain-Vault/oracle-brain/) to see what's already covered
2. For each topic: do 6-10 web queries, extract 3-5 primary sources
3. Write a comprehensive Oracle wiki entry using OKF format
4. Run `brain_sync.py` after writing to update embeddings
5. Move to next topic in queue

## RULES
- Always check what's already been written to avoid duplication
- Focus on what's PRACTICALLY useful for our AI agent system
- Cite primary sources
- Write for an intelligent but non-specialist audience
- Keep entries actionable: what should we DO with this information?

## Response

## Oracle Wiki Expansion Complete

### Knowledge Representation Domain — Priority 1 Topics #8–#14

I've researched and written **7 comprehensive Oracle wiki entries** on the Ontology & Knowledge Systems topics from the research queue:

| # | Topic | File | Size |
|---|-------|------|------|
| 8 | Knowledge Graph Construction | `Knowledge-Graph-Construction.md` | 9,172B / 176 lines |
| 9 | Formal Concept Analysis & Lattice Theory | `Formal-Concept-Analysis-Lattice-Theory.md` | 7,064B / 121 lines |
| 10 | Semantic Web Technologies | `Semantic-Web-Technologies.md` | 7,543B / 147 lines |
| 11 | Taxonomy vs Ontology vs KG | `Taxonomy-vs-Ontology-vs-KG.md` | 6,472B / 143 lines |
| 12 | Commonsense Knowledge | `Commonsense-Knowledge.md` | 6,376B / 149 lines |
| 13 | Ontology Evaluation | `Ontology-Evaluation.md` | 6,408B / 140 lines |
| 14 | Lessons from Cyc, WordNet, FrameNet | `Lessons-from-Cyc-WordNet-and-FrameNet.md` | 7,704B / 147 lines |

**Total:** ~49,740 bytes, 954 lines of new content across 7 files, plus updated `index.md` with full cross-referencing.

### Vault Coverage Analysis (prior to writing)

- **#8 Knowledge Graph Construction:** Partially covered — GNN page mentioned TransE/RotatE; OaK covered chunk-map-merge; Graph-RAG covered entity extraction. Missing: comprehensive pipeline architecture, LLM-based extraction, schema induction.
- **#9–#14:** No dedicated coverage existed in the vault.

### Research Methodology

- **30+ web searches** across all 7 topics using diverse query strategies
- **Extracted primary sources** including recent arXiv papers (2024–2026): Lattice Representation Hypothesis (2026), Formal Concept Lattices as Semantic Scaffolds (2026), Machine Learning and Data Analysis Using Posets Survey (2024), and multiple surveys on KG construction/LLM-based extraction
- **Each entry includes:** "What To Do With This" section with actionable recommendations for the Autognosia system
- **Cross-referenced** with existing vault entries (Structured-Representations-Discrete-Latents, Semantic-Networks-Spread-Activation, Ontology-as-Kernel-OaK)

### Key Findings Worth Highlighting

1. **LLMs encode concept lattices** — The 2026 Lattice Representation Hypothesis shows LLM embeddings contain recoverable symbolic concept hierarchies (87% accuracy on animal hierarchy reasoning)
2. **Semantic Web: relevant but diminished** — RDF/SPARQL remains useful for external data exchange; OWL reasoners are too slow for scale; property graphs (Neo4j) preferred for internal storage
3. **Hybrid extraction pipeline** — LLM for schema induction + traditional models for volume processing is the recommended approach for Autognosia
4. **Formal Concept Analysis as scaffolding** — FCA lattices provide principled semantic structure for neural network learning at different depth levels

### Note on brain_sync

Brain sync was attempted but timed out — Ollama is not running on this machine (only `brain-postgres` is available). The embedding sync will occur during the next scheduled cron cycle when Ollama is active.
