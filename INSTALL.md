...
> **Existing SearXNG instance?** If you already run SearXNG from another
> checkout, verify its JSON API before relying on it:
> `curl -sf "http://127.0.0.1:8080/search?q=test&format=json"`.
> A 403 means `settings.yml` lacks `search: formats: [html, json]` — Hermes's
> web_search will silently fail. The repo's template at
> `docker/core-config/settings.yml` is pre-configured correctly.
...
### 4. Set Up Graphify (Knowledge Graph Index)
...
# Run extraction against a local LLM endpoint. Graphify reads standard
# OpenAI-compatible env vars:
export OPENAI_BASE_URL=http://<your-llm-server>:8080/v1   # LAN llama.cpp/vLLM/LM Studio
export OPENAI_MODEL=<model-name-your-server-serves>
# Reasoning/thinking models (e.g. Qwen3) burn output tokens on hidden
# reasoning before writing JSON — raise the cap so chunks don't truncate:
...
### 5. Set Up Honcho (Autobiographical Memory)
...
### 6. Set Up SessionDB FTS (Full-Text Search)
...
```bash
# Comprehensive verification (14 checks)
...
Start the local web dashboard (Port 8088). The dashboard binds to `0.0.0.0` (all interfaces) by default:

```bash
python3 scripts/run_dashboard.py
```

**Local access:** Open `http://127.0.0.1:8088` in your browser.

**Remote/LAN access:** If the dashboard is running on a VM or remote server, open `http://<host-ip>:8088/` from any machine on the same network. The dashboard binds to all interfaces (`0.0.0.0`), so no extra binding config is needed.

> **Firewall note (UFW):** If the host runs Ubuntu with UFW enabled, port 8088 is blocked by default. Allow it:
> ```bash
> sudo ufw allow 8088/tcp
> ```
> SSH (port 22) is allowed by default; all other ports must be explicitly opened.
