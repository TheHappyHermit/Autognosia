...
* interactive web dashboard & copilot (`http://<host-ip>:8088` — LAN-accessible);
...
* **`verification_checks`**: Intended state vs. observed reality checks (`operation_id`, `expected_result`, `actual_result`, `passed`, `notes`).
* **`routing_events`**: Profile dispatch decisions (`input_summary`, `routed_to`, `route_reason`, `confidence`, `outcome`).
* **`skill_events`**: Procedural skill executions and outcomes (`skill_name`, `trigger`, `success`, `duration_ms`, `error_message`).
* **`reflections`**: Evidence-gated lessons, warnings, and pattern recognitions (`reflection_type`, `content`, `applied`, `source_operation_id`).
* **`key_decisions`**: Critical architecture and strategic decisions with rationale and alternatives (`decision`, `rationale`, `alternatives_considered`, `outcome`, `superseded_by`).
* **`prospective_log`**: Intention triggers and lifecycle executions (`cue`, `action_taken`, `triggered`, `triggered_at`).
...
The Command Deck automatically starts as a background service during repository setup (`scripts/auto_setup.sh` or through the standard setup prompt). To launch manually at any time:
```bash
