# Attribution

This file credits the upstream projects, design patterns, and components that Autognosia integrates.

---

## Integrated & Upstream Technologies

### 1. Hermes Agent
- **Organization:** [Nous Research](https://nousresearch.com/)
- **Repository:** [NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent)
- **Role:** Autonomous agent runtime, procedural memory (skills), execution profiles, and tool orchestration.

### 2. Honcho
- **Organization:** [Plastic Labs](https://plasticlabs.ai/)
- **Repository:** [plastic-labs/honcho](https://github.com/plastic-labs/honcho)
- **Role:** Autobiographical memory, user modeling, and dialectic state derivation.

### 3. Brain Search (Postgres + pgvector)
- **Creator:** Built-in (inspired by Graphify)
- **Role:** Markdown-first historical knowledge graph, hybrid vector retrieval (pgvector), and MCP endpoint.

### 4. Graphify
- **Organization:** [Graphify-Labs](https://github.com/Graphify-Labs) / [Safi Shamsi](https://github.com/safishamsi)
- **Repository:** [safishamsi/graphify](https://github.com/safishamsi/graphify)
- **Role:** AST and semantic knowledge graph extraction over Active Wiki and Oracle Vault.

### 5. SearXNG
- **Organization:** [SearXNG Community](https://searxng.org/)
- **Repository:** [searxng/searxng](https://github.com/searxng/searxng)
- **Role:** Privacy-respecting metasearch engine providing isolated web retrieval for the Researcher profile.

### 6. LLM-Wiki Pattern
- **Creator:** [Andrej Karpathy](https://karpathy.ai/)
- **Source:** [Karpathy LLM-Wiki Gist](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)
- **Role:** Crystallized semantic memory format with Markdown pages, frontmatter schemas, and ingestion logs.

---

## License Summary

Autognosia is licensed under the MIT License. Upstream projects retain their respective copyrights and licenses:
- Hermes Agent: Apache 2.0 / MIT
- Honcho: MIT
- Brain Search: MIT
- Graphify: MIT
- SearXNG: AGPL 3.0
