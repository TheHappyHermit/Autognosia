...
4. Store the token in `${HERMES_HOME:-${HOME}/.hermes}/.env` (or via `hermes setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `${HERMES_HOME:-${HOME}/.hermes}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
