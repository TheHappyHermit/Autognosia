...
Config: `${HOME}/.hermes/config.yaml` (settings), `${HOME}/.hermes/.env` (API keys) — both under `$HERMES_HOME` when it is set.
...
never hardcode `${HOME}/.hermes`. For custom/local-only tools, write a plugin in
`${HOME}/.hermes/plugins/` instead of editing core — see the developer docs.
