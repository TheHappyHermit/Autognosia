...
Add to `${HERMES_HOME:-${HOME}/.hermes}/.env`:
```bash
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `${HOME}/.hermes/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `${HOME}/.hermes/scripts/` (path traversal is blocked).
...
- Subscriptions persist to `${HOME}/.hermes/webhook_subscriptions.json`

## How It Works

1. `hermes webhook subscribe` writes to `${HOME}/.hermes/webhook_subscriptions.json`
...
3. **Check gateway logs:** `grep webhook ${HOME}/.hermes/logs/gateway.log | tail -20`
