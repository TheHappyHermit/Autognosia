...
system (`${HOME}/.hermes/plugins/`) is otherwise documented separately.
...
1. Create `$HERMES_HOME/desktop-plugins/<name>/plugin.js` from
   ...
   `${HOME}/.hermes/...` by default, or `${HOME}/.hermes/profiles/<profile>/...` under a
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `${HOME}/.hermes/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
