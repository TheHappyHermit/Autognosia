...
`${HOME}/.hermes/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- Write access to the Hermes home dir — `${HOME}/.hermes` by default, or `$HERMES_HOME`
...
- **Don't hardcode `${HOME}/.hermes`** when a profile is active — resolve the real home
  from `$HERMES_HOME` first, falling back to `${HOME}/.hermes`.
