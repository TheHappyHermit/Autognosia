...
**When to use**: The FreshRSS hostname is NXDOMAIN, not in `/etc/hosts`, or only resolves on a different network segment. Use the raw server IP (e.g., `10.1.1.10`) with the correct `Host` header (e.g., `freshrss.[private-site].com`).
