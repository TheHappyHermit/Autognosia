...
- Don't use for: personal skills in `${HOME}/.hermes/skills/` (just use `skill_manage`)
...
- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `${HOME}/.hermes/skills/`.
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `${HOME}/.hermes/skills/`, not the repo tree. Use `write_file`.
